...
bool Eth_Init(void)
{
    ...
    /* MAC RMII Config (Pass All Removed to rely on MAC HW) */
    ...
    /* --- FORCE 10 Mbps FULL DUPLEX (Bypass Hardware Noise) --- */
    Eth_WritePHY(DP83848_BMCR, 0x0100);
    for (timeout = 0; timeout < 0xFFFFF; timeout++);

    /* Hardcode MAC to match the PHY */
    LPC_EMAC->MAC2 |= MAC2_FULL_DUP;
    LPC_EMAC->COMMAND |= CR_FULL_DUP;
    LPC_EMAC->IPGT = 0x0015;

    // SUPP Bit 8 = 0 forces the LPC2478 MAC to 10 Mbps
    LPC_EMAC->SUPP = 0;
    /* --------------------------------------------------------- */
