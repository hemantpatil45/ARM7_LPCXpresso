...
void PLL_Init(void)
{
    ...
    // CRITICAL FIX 1: Wait for the oscillator to stabilize!
    // Do not use a simple delay here.
    while((SCS & 0x40) == 0);

    // 2. Select Main Oscillator as PLL clock source
    CLKSRCSEL = 0x01;

    // 3. Configure PLL for 72MHz (Assuming 12MHz physical crystal)
    // M = 12 (MSEL = 11), N = 1 (NSEL = 0)
    PLLCFG = 0x0000000B;
    PLLFEED = 0xAA;
    PLLFEED = 0x55;
    ...
    // CRITICAL FIX 2: Wait for PLL to lock!
    // Never proceed until bit 26 of PLLSTAT is 1.
    while((PLLSTAT & (1 << 26)) == 0);

    // 5. Connect PLL and Set CPU clock divider
    CCLKCFG = 0x03; // CCLK = Fcco / 4 = 288/4 = 72MHz
    PLLCON = 0x03;  // Enable and Connect PLL
    PLLFEED = 0xAA;
    PLLFEED = 0x55;

    // 6. Set Peripheral Clock to CCLK/4 (18MHz)
    PCLKSEL0 = 0x00000000;
    PCLKSEL1 = 0x00000000;

    // CRITICAL FIX 3: Memory Accelerator Module (MAM)
    // Flash memory cannot keep up with 72MHz without this configuration.
    MAMCR = 0;       // Disable MAM temporarily
    MAMTIM = 3;      // Set 3 fetch cycles (Required for 72MHz)
...
//**************************PINMODE (ALL PINS ARE PULL UP REGISTER ENABLED)*******
    PINMODE0 = 0x00000000;
   	PINMODE1 = 0X00000000;
   	PINMODE2 = 0xAAAAAAAA; // 0xAA disables pull-ups for pristine 50MHz RMII timing   	PINMODE3 = 0X00000000;
   	PINMODE4 = 0X00000000;
   	PINMODE5 = 0XAA02022A;
