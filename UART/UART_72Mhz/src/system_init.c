...
void PLL_Init(void)
{
    // 1. Enable Main Oscillator (Bit 5) and Fast GPIO (Bit 0)
    SCS = 0x21;

    // CRITICAL: Wait for the hardware crystal to physically stabilize
    while ((SCS & 0x40) == 0);

    // 2. Select Main Oscillator as the PLL clock source
    CLKSRCSEL = 0x01;

    // 3. Configure PLL for 72MHz CCLK (Assuming 12MHz Crystal)
    // Formula: FCCO = (2 * M * Fin) / N
    // Target FCCO = 288 MHz.  Fin = 12 MHz. Therefore: M = 12, N = 1
    // MSEL = M - 1 = 11 (0x0B).  NSEL = N - 1 = 0.
    PLLCFG = 0x0000000B; // Use '=' not '|=' to ensure a clean overwrite
    ...
    // CRITICAL: Wait for the PLL to achieve frequency lock
    while ((PLLSTAT & (1 << 26)) == 0);

    // 5. Set CPU Clock Divider (FCCO / 4 = 288MHz / 4 = 72 MHz)
    CCLKCFG = 0x03;

    // 6. Connect the locked PLL to the CPU core
    PLLCON = 0x03;
    PLLFEED = 0xAA;
    PLLFEED = 0x55;

    // 7. Set Peripheral Clocks to CCLK / 4 (72 / 4 = 18 MHz for UART)
    PCLKSEL0 = 0x00000000;
    PCLKSEL1 = 0x00000000;
}
