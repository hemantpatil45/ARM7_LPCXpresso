...
int main(void) {
    ...
    // Give power rails, PHY, and clocks time to settle
    // ==========================================
    delay_ms(5000);
    ...
    while (1) {
        if (g_USBConnected) {
            ...
            if (fr == FR_OK) {
                ...
                // Explicitly targeting HOST_LOG.TXT
                fr = f_open(&TestFile, "0:/VIJAYA.TXT", FA_WRITE | FA_OPEN_ALWAYS);

                if (fr == FR_OK) {
                    UART0_Print("[MAIN] Writing test string to file...\r\n");
                    f_write(&TestFile, "Welcome to vijaya    Lab Solution USB host implemented successfully\r\n", 67, &bytesWritten);
                    f_close(&TestFile);
                    UART0_Print("[MAIN] Write complete and file closed.\r\n");
                } else {
                    // ==========================================
                    // ERROR CODE PRINTER
                    // Prints the exact FRESULT code to terminal
                    // ==========================================
                    UART0_Print("[MAIN_ERR] Failed to open file! FatFs Error Code: ");
                    if (fr >= 10) {
                        UART0_SendChar((fr / 10) + '0');
                        UART0_SendChar((fr % 10) + '0');
                    } else {
                        UART0_SendChar(fr + '0');
                    }
