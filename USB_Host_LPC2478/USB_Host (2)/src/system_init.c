...
void PLL_Init(void) {
    // 1. Disconnect and Disable the PLL before changing it
    if (PLLSTAT & (1 << 25)) {
        PLLCON = 1;               // Enable only
        PLLFEED = 0xAA; PLLFEED = 0x55;
    }
    PLLCON = 0;                   // Disable PLL
    PLLFEED = 0xAA; PLLFEED = 0x55;

    // 2. Enable Main Oscillator and wait for it to be ready
    SCS |= 0x20;
    while (!(SCS & 0x40));

    // 3. Select Main Oscillator (12MHz) as the PLL clock source
    CLKSRCSEL = 0x01;

    // 4. M=12, N=1 -> Fcco = (2 * 12 * 12MHz) / 1 = 288 MHz
    PLLCFG = 0x0000000B;
    PLLFEED = 0xAA; PLLFEED = 0x55;
    ...
    // ========================================================
    // CRITICAL FIX: Wait for the PLL to achieve phase lock!
    // ========================================================
    while (!(PLLSTAT & (1 << 26)));

    // 6. CPU Clock = 288 / 6 = 48 MHz
    CCLKCFG = 0x05;

    // 7. USB Clock = 288 / 6 = 48 MHz (Required by USB PHY)
    USBCLKCFG = 0x05;
    ...
    // Wait for PLL to report as connected
    while (!(PLLSTAT & (1 << 25)));
...
//***********************ETHERNET SECTION***************************************
    ...
    //**********************SET TO USB2 HOST (CORRECTED MASKING) ***********************
        PINSEL0 = (PINSEL0 & ~(3UL << 24)) | (1UL << 24); // M_USB_PPWR2 P0[12]
        PINSEL0 = (PINSEL0 & ~(3UL << 26)) | (1UL << 26); // M_USB_UP_LED2 P0[13]
        PINSEL1 = (PINSEL1 & ~(3UL << 30)) | (1UL << 30); // M_USB_D+2 P0[31]
        PINSEL3 = (PINSEL3 & ~(3UL << 28)) | (1UL << 28); // M_USB_PWRD2 P1[30]
        PINSEL3 = (PINSEL3 & ~(3UL << 30)) | (1UL << 30); // M_USB_OVRCR2# P1[31]
