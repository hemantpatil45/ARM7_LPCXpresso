...
void UART0_SendHex8(uint8_t value)
{
...
void UART0_SendDec(uint32_t value)
{
    char buf[12];
    int i = 0;
...
int main(void) {
    ...
    UART0_SendString("MCFG = ");
    UART0_SendHex32(LPC_EMAC->MCFG);

    UART0_SendString("\r\n");

    UART0_SendString("MADR = ");
    UART0_SendHex32(LPC_EMAC->MADR);

    UART0_SendString("\r\n");

    UART0_SendString("RXFILTERCTRL = ");
    UART0_SendHex32(LPC_EMAC->RXFILTERCTRL);

    UART0_SendString("\r\n");
    UART0_SendHex32(PINSEL2);
    UART0_SendString("\r\n");

    UART0_SendHex32(PINSEL3);
    UART0_SendString("\r\n");

    UART0_SendHex32(PINMODE2);
    UART0_SendString("\r\n");

    UART0_SendHex32(FIO1DIR);
    UART0_SendString("\r\n");
    UART0_SendString("COMMAND = 0x");
    UART0_SendHex32(LPC_EMAC->COMMAND);

    UART0_SendString("\r\nMAC1 = 0x");
    UART0_SendHex32(LPC_EMAC->MAC1);

    UART0_SendString("\r\nMAC2 = 0x");
    UART0_SendHex32(LPC_EMAC->MAC2);

    UART0_SendString("\r\nSUPP = 0x");
    UART0_SendHex32(LPC_EMAC->SUPP);
